...
(function ($) {
  ...
  Drupal.atomizer.ptableC = function (_viewer) {
     ...
     * @param results
     */
    var doCreateObject = function doCreateObject (results) {
      var az;
      for (var i = 0; i < results.length; i++) {
        ...
        if (result.command == 'loadNodeCommand') {
          ...
          pte = createScene(result.data.nodeConf);
          pte.az = {
            nid: result.data.nid,
          ...
          // Move pte position
          if (viewer.dataAttr['pte--position--x']) {
            if (!pte.position) pte.position = new THREE.Vector3();
            pte.position.x = viewer.dataAttr['pte--position--x'];
          }
          if (viewer.dataAttr['pte--position--y']) {
            if (!pte.position) pte.position = new THREE.Vector3();
            pte.position.y = viewer.dataAttr['pte--position--y'];
          }
          if (viewer.dataAttr['pte--position--z']) {
            if (!pte.position) pte.position = new THREE.Vector3();
            pte.position.z = viewer.dataAttr['pte--position--z'];
          }

//        if (!pte.rotation) pte.rotation = new THREE.Vector3();
//        pte.rotation.x =  30 / 360 * 2 * Math.PI;
//        pte.rotation.z = -45 / 360 * 2 * Math.PI;
    ...
    var createScene = function createScene (conf) {
      ...
      function init() {
        ...
        for ( var i = 0; i < table.length; i += 5 ) {
          ...
          cell.style.backgroundColor = 'rgba(0,127,127,' + ( Math.random() * 0.5 + 0.25 ) + ')';
          ...
          var element = new THREE.CSS3DObject( cell );
//        element.position.x = Math.random() * 4000 - 2000;
//        element.position.y = Math.random() * 4000 - 2000;
//        element.position.z = Math.random() * 4000 - 2000;

          element.position.x =  (table[i + 3] * 140) - 1330;
          element.position.y = -(table[i + 4] * 180) + 1200;
          element.position.z = 0;

          viewer.scene.add(element);
...
//      viewer.canvasContainer.appendChild(viewer.renderer.domElement);
      ...
      function transform(locations, duration) {
        TWEEN.removeAll();
        ...
        for (var i = 0; i < elements.length; i++) {
          ...
          new TWEEN.Tween(element.position)
              .to({x: loc.position.x, y: loc.position.y, z: loc.position.z}, Math.random() * duration + duration )
              .easing(TWEEN.Easing.Exponential.InOut)
              .start();

//        new TWEEN.Tween(element.rotation)
//            .to( {x: loc.rotation.x, y: loc.rotation.y, z: loc.rotation.z}, Math.random() * duration + duration )
//            .easing(TWEEN.Easing.Exponential.InOut)
//            .start();
        }
...
//    for (var cylinder in pte.cylinders) {
//      var conf = pte.cylinders[cylinder].conf;
//    }
      viewer.render();
    }
     ...
     * @param pte
     */
    var deleteScene = function deleteScene (pte) {
      viewer.scene.remove(pte);
    };
